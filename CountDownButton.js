...
class CountDownButton extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            timerTitle: this.props.timerTitle || getString('getVerifyCode',this.props.curLanguageIndex),
            counting: false,
            selfEnable: true,
        };
    ...
    _countDownAction(){
        ...
        this.interval = setInterval(() =>{
            ...
            if (nowStamp >= overTimeStamp) {
                ...
                this.setState({
                    timerCount: codeTime,
                    timerTitle: timerTitle || getString('getVerifyCode',this.props.curLanguageIndex),
                    counting: false,
                    selfEnable: true
                })
                ...
                let activeTitle = `${getString('reGetVerifyCode',this.props.curLanguageIndex)}(${leftTime}s)`
                if (timerActiveTitle) {
        ...
        return (
            <TouchableOpacity activeOpacity={counting ? 1 : 0.8} onPress={()=>{
                ...
                <View style={[{width:120, height:44,justifyContent:'center',alignItems:'center'},style]}>
                    <Text allowFontScaling={false} style={[{fontSize: 16},textStyle,{color: ((!counting && enable && selfEnable) ? (textStyle.color ? textStyle.color : '#606eff') : disableColor || 'gray')}]}>{timerTitle}</Text>
                </View>
            </TouchableOpacity>
        )
    }
...
export default connect(mapStateToProps)(CountDownButton)