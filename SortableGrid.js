...
class SortableGrid extends Component {
    ...
    _getGridStyle = () => [
        styles.sortableGrid,
        this.props.style,
        this._blockPositionsSet() && { height: (this.rows+1) * (this.props.itemHeight + 10)}
    ]

    _getDeletionView = (key) => {
        if (this.state.deleteModeOn)
            return <Image style={ this._getImageDeleteIconStyle(key) } source={require('../data/img/edit_delete.png')} />
    }
